      ...
      const res = await fetch('/api/advisor-chat', {
        ...
        body: JSON.stringify({
          question: contextMode === 'estudo'
            ? `Conecte o cenário atual (SELIC ${marketData?.macro?.selic?.value ?? '?'}%, IPCA ${marketData?.macro?.ipca?.value ?? '?'}%, B3) com os 8 módulos do currículo IPB. Use os dados reais como caso prático de cada módulo. Cite autores. Termine com 3 perguntas desafiadoras.${iaModifier}`
